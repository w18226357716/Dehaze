...
parser.add_argument('--dataset', type=str, default='SOTS', help='Path of the validation dataset')  # 验证集的路径
parser.add_argument("--checkpoint", default="", type=str, help="Test on intermediate pkl (default: none)")
parser.add_argument('--gpu_ids', type=str, default='0', help='gpu ids: e.g. 0  0,1,2, 0,2. use -1 for CPU')
parser.add_argument('--name', type=str, default='Ours-all', help='filename of the training models')
...
def test(test_gen, model, criterion, SR_dir):
    ...
    with torch.no_grad():
        for iteration, batch in enumerate(test_gen, 1):
            ...
            print("Processing {}:  PSNR:{:.3f}   SSIM:{:.4f}".format(iteration, psnr, ssim))
            avg_ssim += ssim
            avg_psnr += psnr
